...
int main(int argc, char** argv)
{
    ...
    // Initialize device arrays d_A and d_B
    int i;
    for(i=0; i<N; ++i){
     d_A[i] = (float)i;
     d_B[i] = (float)(N-i);   
    ...
    // prefetch device vectors d_A, d_B, d_C
    cudaMemPrefetchAsync(d_A, N*sizeof(float), 0, 0);
    cudaMemPrefetchAsync(d_B, N*sizeof(float), 0, 0);
    cudaMemPrefetchAsync(d_C, N*sizeof(float), 0, 0);
